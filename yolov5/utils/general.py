    ...
    for i, d in enumerate(x):  # per image
        if d is not None and len(d):
            ...
            for j, a in enumerate(d):  # per item
                ...
                # cv2.imwrite('example%i.png' % j, cutout)
...
def save_one_box(xyxy, im, file='image.png', gain=1.02, pad=10, square=False, BGR=False, save=True):
    ...
    if save:
        cv2.imwrite(str(increment_path(file, mkdir=True).with_suffix('.png')), crop)
    return crop
